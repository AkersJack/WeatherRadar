...
# CURR_DIR = "/home/james/Weather-Radar/Examples/"
...
def location_to_station():
    ...
    point_url = f"https://api.weather.gov/points/{39.399283},{-77.061142}" # Lat, Long (Home)
    # point_url = f"https://api.weather.gov/points/{37.727469},{-89.216655}" 
    # point_url = f"https://api.weather.gov/points/{53.914616},{-122.736511}"
...
def get_radar_images(base_map_layer='osm', layer=None, zoom=None, show_alerts=True, warnings_list=[], hazard_list=[], frames=None):
    # Your existing get_radar_images function...
    # Modify to return the final combined image instead of a list
...
#def get_basemap(mode,zoom,width,provider='osm'):
...
def get_basemap(mode,zoom,width, height, provider='osm'):
    ...
    if mode == "coordinate":
        ...
        # map = geotiler.Map(center=(-6.069, 53.390), :w=16, size=(512, 512))
        #map.extent
        image = geotiler.render_map(map) 
        # white = (255, 255, 255)
        # screen.fill(white)
        update_display(image) # This works
...
def status_images(message,background=None, background_colour=(100,100,100,200), font=fnt_goth_bold, font_colour=(255,255,255,255), xy=None, border=False):
    ...
    #disp.image(status_images("Standby!",^))
###########################$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$##########################################

    
    # At the end of the function:
    #return combined  # Return the final combined image
...
def main():
    ...
    while running:
        ...
        ### If there's an error, get the time, and display it.
                ...
                screen.image(status_images(message,loading,font=fnt_goth_medium))
...
def testText():
    ...
    WHITE = (255, 255, 255)
    # screen.fill(WHITE)
    # text_surface = font.render("Testing", True, BLACK)
...
def config():
    if GPIO_AVAILABLE == True:
        GPIO.setup(zoomButton, GPIO.IN, pull_up_down=GPIO.PUD_UP)
        GPIO.setup(modeButton, GPIO.IN, pull_up_down=GPIO.PUD_UP)
        radars = ["sr_bref", "sr_bvel", "boha", "bdsa", "bdhc"]
        counter = 0
        while True:
            if not GPIO.input(modeButton):
                print('zoom Input was LOW') 
                settings.zoom = (settings.zoom % 13) + 1
                if settings.zoom < 8: 
                    settings.zoom = 8
                settings.stop = True
                settings.images = []
                time.sleep(2)
                # print('zoom Input was HIGH')
            #else:
            if not GPIO.input(zoomButton):
                #print('mode Input was HIGH')
                print('mode Input was LOW')
                counter = (counter % 4) + 1
                settings.radar = radars[counter]
                settings.stop = True
                settings.images = []
                time.sleep(2)
        ...
        user_input = input("Enter Command: ")
            ...
            #    settings.zoom = int(user_input.split('zoom ')[1])
            #else:    
            #    settings.radar = user_input
...
if __name__ == "__main__":
    ...
    # Set up the display
    # screen = pygame.display.set_mode((320, 240), pygame.FULLSCREEN)  # Adjust size as needed
    # screen = pygame.display.set_mode((1024, 768), pygame.FULLSCREEN)  # Adjust size as needed
    screen = pygame.display.set_mode((1024, 768))  # Adjust size as needed
    ...
    # loading = Image.open(f"{CURR_DIR}\\Examples\\loading.png")
