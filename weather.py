...
# Only works if you have GPIO pins


# Globals 
GPIO_AVAILABLE = True
# global LOCAL_WARNINGS
# global LOCAL_ALERTS
# global TIMEZONE
# global STATION 
global LAYER
...
# Test for GPIO (if we have GPIO pins to use)
try:
    import RPi.GPIO as GPIO
    # Set up the GPIO mode
    GPIO.setmode(GPIO.BCM)
    
    # Set up a channel as an output
    ...
    print("No GPIO pins detected!")
    GPIO_AVAILABLE = False


# The actual config of the program (things that apply before run)

class Config: 
    def __init__(self): 
        self.curr_dir = f"{os.getcwd()}/"
        self.headers = secrets['header']
        ...
        self.fnt_small = ImageFont.truetype(f"{self.curr_dir}HelveticaNeue.ttf", 10)
        self.fnt_medium = ImageFont.truetype(f"{self.curr_dir}HelveticaNeue.ttf", 15)
        self.fnt_goth = ImageFont.truetype(f"{self.curr_dir}CenturyGothic.ttf", 20)
        self.fnt_goth_bold = ImageFont.truetype(f"{self.curr_dir}CenturyGothic-Bold.ttf", 20)
        self.fnt_goth_medium = ImageFont.truetype(f"{self.curr_dir}CenturyGothic.ttf", 15)
...
# layer = settings.radar


# The settings of the program (currently running)
class Settings(): 
    def __init__(self):
        self.radars = ["sr_bref", "sr_bvel", "boha", "bdsa", "bdhc"]
        self.radar_counter = 0
        self.radar = "sr_bref"
        self.layer = 'bohp' # prob change to self.radar 
        self.location = ()
        #self.zoom = 8
        self.zoom = 8
        self.images = []
        self.stop = False
        self.running = True
        self.lat_long = secrets['coordinates']
        #self.point_url = f"https://api.weather.gov/points/{39.399283},{-77.061142}" # Lat, Long (1)
        self.point_url = f"https://api.weather.gov/points/{38.2921588},{-77.4208659}" # Lat, Long (2)
        self.provider = 'osm'
        self.providers = geotiler.providers()
...
def location_to_station():
    ...
    '''
    try:
        response = requests.get(settings.point_url, headers=config.headers, timeout=5)
    except:
...
def get_station_data(station_param=None): 
    ...
    while (response == False):
        try: 
            response = requests.get(stations_url, headers=config.headers, timeout=20)
        except: 
            print("Connection Problems: Getting radar station data")
            response = False
            # Wait 10 seconds before trying the connection again 
            time.sleep(10)
...
"""
    Updates the display with the given image and a list of integers.
    
    Args:
      image: The image to display.
      rect: A tuple of integers, required to have a length of 2.
    
    Returns:
      None
"""

def update_display_fast(image, rect: tuple[int, int]):
    # Convert PIL Image to Pygame surface 
    mode = image.mode 
    size = image.size 
    data = image.tobytes()
...
def get_times(url): 
    ...
    if response: 
        ...
        try: 
            ...
            times = capabilties_dict["WMS_Capabilities"]["Capability"]["Layer"]["Layer"][0]["Dimension"]["#text"]
...
# def get_basemap(mode, zoom, width, height, provider = 'osm'):
def get_basemap(mode, zoom, width, height, provider):
    ...
    if mode == "coordinate":
        ...
        #                     size=size,
        #                     provider=provider)
        # map = geotiler.Map(center=(39.400547, -77.059996), zoom=zoom, size=size)
        #map = geotiler.Map(center=(-77.059996, 39.400547), zoom=zoom, size=size)
        print(f"zoom: {zoom}")
        print(f"providers: {geotiler.providers()}")
        print(f"Radar: {settings.radar}")
        
        #map = geotiler.Map(center=(-77.061142, 39.399283), zoom=settings.zoom, size=size) # Home 
        map = geotiler.Map(center=(-77.4208659, 38.2921588), zoom=settings.zoom, size=size) # Home 
        #map = geotiler.Map(center=(-89.216655, 37.727469), zoom=settings.zoom, size=size) # Long, Lat
        # map.provider = geotiler.find_provider('osm')
        map.provider = geotiler.find_provider(provider)
...
def get_radar_images(base_map_layer= 'osm', layer=None, zoom=None, show_alerts=True, warnings_list=[], hazard_list=[], frames=None):
    ...
    base_map, map = get_basemap("coordinate",provider=base_map_layer,zoom=settings.zoom,width=1024, height=768)
    size = map.size
    ...
    ###################################################
    #'https://opengeo.ncep.noaa.gov/geoserver/wwa/hazards/ows?service=wms&version=1.3.0&request=GetCapabilities'
    ...
    for i,image in enumerate(prev_images):
        ...
        radar_url = f"https://opengeo.ncep.noaa.gov:443/geoserver/{STATION}/ows?SERVICE=WMS&service=WMS&version=1.3.0&request=GetMap&layers={STATION}_{settings.radar}&styles=&width={size[0]}&height={size[1]}&crs={SRS}&bbox={bbox}&format={format}&transparent={transparent}&bgcolor={bg_colour}&exceptions={EXCEPTION}&time={TIME_for_url}"
        # print(f"Radar_url: {radar_url}")
        response_radar = False
        
        while (response_radar == False):
            try: #Try to get the radar image
                response_radar = requests.get(radar_url, headers = config.headers, timeout = 10)
        ...
        if time_since < 10: #Add a filling zero if less than 10
            ...
            filler = ""
        datetime_string = f"{datetime_string} ({filler}{time_since} mins)" # HERE 

        #Centre the time based on text length.
        text_length = combined_annotation.textlength(datetime_string,font=config.fnt_medium)
        ...
        if len(LOCAL_ALERTS[0]) > 0:
            ...
            for a_hazard_type in unique_hazards:
                ...
                if len(unique_hazards) >= 3:
                    alert_font = config.fnt_small
                    y_offset = 15
                else:
                    alert_font = config.fnt_medium
                    y_offset = 20
...
def get_all_alerts(*hazard_types, coordinates = None): 
    ...
    hazard_time = hazard_time[-1] # Get the latest time 
    
    warning_time, warning_datetime = get_times(warnings_capabilities_url)
    warning_time = warning_time[-1] # Get the latest time
    ...
    while (response_warning == False):
        try: #Warnings
            response_warning = requests.get(warning_json_url, headers=config.headers,timeout=5)
        except:
            print("Connection problem getting warning file.")
            response_warning = False
            time.sleep(10)

    while (response_hazard == False):
        try: #Hazards
            response_hazard = requests.get(hazard_json_url, headers=config.headers,timeout=5)
        except:
...
'''
event.pump() for internal pygame operations 
Use to: 
    - Internally process the event queue without removing any events 
    - Doesn't retrieve or process events 
    - Use to handle custom input methods or handling display without processing
      events directly 
    - Processes internal event handlers to keep everything current
    - Use to update the current state of the system and you don't need to know events

event.get() for retrieving a list of all currently available events from the event queue.
Use to: 
    - handle user input
    - react to specific events

event.get() clears the queue after returning the events 

It is possible for the queue to get filled up and events to be dropped
    ...
    for frame in frames:
        ...
        while time.monotonic() < (start_time + duration / 1000):
            ...
            if settings.stop == True: 
                print("Getting out of animation")
                return None
...
def status_images(message ,background = None, background_colour = (100,100,100,200), font = fnt_goth_bold, font_colour = (255,255,255,255), xy = None, border = False):
    ...
    if border:
        circle_overlay = Image.open(f"{config.curr_dir}circle_overlay.png")
    ...
    while settings.running: 
        for event in pygame.event.get(): 
            try:
                ...
                print(f"\n----------------------\n     Local info:\n----------------------")
                LOCAL_WARNINGS, LOCAL_ALERTS = get_all_alerts(coordinates=(settings.lat_long[1],settings.lat_long[0]))
        ...
        ##############################
                if get_station_data(STATION) in ["Up","Online"]:
                    ...
                    settings.images = get_radar_images(
                       #  base_map_layer='stamen-toner',
                        base_map_layer = settings.provider,
                        # base_map_layer = 'osm',
                       #  layer="bohp",
                        # layer="sr_bref",
                        layer = settings.radar,
                       #  layer="bdhc", # could use sr_bvel as well
                        zoom=settings.zoom,
                        show_alerts=True,
        ...
        ##############################
                while time.monotonic() < start_time + interval:
                    ...
                    # pygame.event.pump()
                    pygame.event.get()
                    play_animation(settings.images)
                    if settings.stop == True:
                        # print("Stopping")
                        break
        ...
        ### If there's an error, get the time, and display it.
                ...
                screen.image(status_images(message,loading,font=config.fnt_goth_medium))
                #status_images(message,background=None, background_colour=(100,100,100,200), font=fnt_goth_bold, font_colour=(255,255,255,255), xy=None, border=False)
                logging.exception('Caught an error')
                break

#   ####################$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$#####################################################

       # radar_image = get_radar_images(...)  # Call with appropriate parameters
                # update_display(radar_image)
        
        # Control the frame rate
                # clock.tick(1/30)  # Update every 5 minutes (300 seconds)
...
if __name__ == '__main__':
    print(geotiler.providers())
    config = Config()
    settings = Settings()
    STATION = location_to_station()
    capabilities_url = f'https://opengeo.ncep.noaa.gov/geoserver/{STATION}/ows/wms?service=wms&version=1.3.0&request=GetCapabilities'
    ...
    loading = Image.open(f"{config.curr_dir}loading.png")
    loading = loading.resize(size=[1024, 768])
    update_display(loading)
    settings.provider = 'thunderforest-cycle'

    

    
    # worker = threading.Thread(target=config)
    # worker.daemon = True 
    # worker.start()
