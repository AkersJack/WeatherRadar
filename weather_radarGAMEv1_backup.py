...
def get_radar_images(base_map_layer='stamen-toner', layer=None, zoom=None, show_alerts=True, warnings_list=[], hazard_list=[], frames=None):
    ...
    base_map, base_map_labels, map = get_basemap("coordinate",provider=base_map_layer,zoom=zoom,width=1280)
    size = map.size
...
def get_basemap(mode,zoom,width,provider='stamen-toner'):
    ...
    if mode == "coordinate":
        ...
        #                     size=size,
        #                     provider=provider)
        map = geotiler.Map(center=(lat_long[1], lat_long[0]), zoom=zoom, size=())
