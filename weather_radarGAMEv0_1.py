...
#GPT says I may need these things
import json
from io import BytesIO
import xmltodict
...
##########################Begin GPT garbage on radar location###################################
...
def location_to_station():
    ...
    global station, forecast_url, timeZone
    ...
    try:
        ...
        point_file = response.json()['properties']
        ...
        station = secrets["station"]  # Fallback station code if request fails
    return station

def get_bounding_coordinates(url):
    ''' Get bounding coordinates of a WMS layer. '''
    try:
        ...
        return (
            float(bounding_coordinates['westBoundLongitude']),
            float(bounding_coordinates['southBoundLatitude']),
            float(bounding_coordinates['eastBoundLongitude']),
            float(bounding_coordinates['northBoundLatitude'])
        )
    except requests.exceptions.RequestException as e:
        print(f"Error fetching bounding coordinates: {e}")
        return 0, 0, 0, 0

station = location_to_station()

# URL for the capabilities file
...
#################################hopefully we have the capabilities file by now################
...
def status_images(message,background=None, background_colour=(100,100,100,200), font_colour=(255,255,255,255), xy=None, border=True):   
